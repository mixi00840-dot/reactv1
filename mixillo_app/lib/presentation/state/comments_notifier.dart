    ...
    return result.fold(
      ...
      (_) {
        ...
        final updatedComments = state.comments.where((c) {
          ...
          // Check if any replies need to be filtered
          final hasReplyToRemove = c.replies.any((reply) => reply.id == commentId);
          return !hasReplyToRemove || c.id != commentId;
        }).map((c) {
          // Filter out the reply if it exists
          final filteredReplies = c.replies.where((reply) => reply.id != commentId).toList();
          
          if (filteredReplies.length != c.replies.length) {
            // A reply was removed, update the parent
            return Comment(
              id: c.id,
              contentId: c.contentId,
              userId: c.userId,
