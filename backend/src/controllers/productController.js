...
class ProductController {
  ...
  async searchProducts(req, res) {
    try {
      ...
      // Delegate to getProducts with search parameter
      req.query = { ...req.query, search: query, ...filters };
  ...
  async bulkUpdateProducts(req, res) {
    try {
      res.status(503).json({
        success: false,
        message: 'Bulk update not yet implemented for Firestore'
      });
    } catch (error) {
  ...
  async updateInventory(req, res) {
    try {
      ...
      let newQuantity = product.inventory?.stockQuantity || 0;
      if (operation === 'add') {
  ...
  async getProductAnalytics(req, res) {
    try {
      // Delegate to getProductStats
      return this.getProductStats(req, res);
    } catch (error) {
